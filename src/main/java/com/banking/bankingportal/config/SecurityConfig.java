...
public class SecurityConfig {
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http.securityContext().requireExplicitSave(false).and()
				...
				.requestMatchers("/admin/**").permitAll().requestMatchers("/customer/**").hasRole("USER")
				.requestMatchers("/register/**").permitAll().requestMatchers("/user").authenticated().and().httpBasic();
		return http.build();
